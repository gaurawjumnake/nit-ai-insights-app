...
class ImportAccountsData:
    ...
    def import_accounts_and_projects_from_file(self, db: Session, content: bytes, filename: str, dry_run: bool = False):
        ...
        for _, row in df.iterrows():
            account = _get_or_create_account(db, row) # type:ignore
    ...
    def get_project_level_summary(self, 
        ...
        project_level_summary = (
            db.query(
                ...
                func.sum(P.expected_revenue).label("total_expected_rev"),
                func.sum(P.ytd_revenue).label("total_ytd_rev"),
                func.sum(P.ai_revenue).label("total_ai_rev"),
                func.sum(P.ai_assisted_revenue).label("total_ai_assist_rev"),
                func.sum(P.ai_direct_hours).label("total_ai_direct_hours"),
                func.sum(P.ai_assist_hours).label("total_ai_assist_hours"),
                func.sum(case((P.status == project_status, 1), else_=0)).label("total_project_count"),
        ...
        response = [
            ProjectSummary(
                ...
                total_project_count=row.total_project_count or 0,
